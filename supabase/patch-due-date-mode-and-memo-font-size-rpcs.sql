-- 支払期限モード / 備考文字サイズ対応の RPC 更新
-- 依存: add-invoice-due-date-mode-and-memo-font-size.sql を先に実行
-- ベース: update-quote-customer-reassign-atomic.sql / add-update-documents-with-items-rpcs.sql
-- 顧客変更原子化を含む update_quote_with_items を維持したまま拡張
...
begin
  ...
  update public.quotes set
    ...
    memo_font_size = coalesce(
      nullif(trim(p_quote->>'memo_font_size'), ''),
      memo_font_size,
      'normal'
    ),
    document_email = case
...
begin
  ...
  update public.invoices set
    ...
    due_date_mode = case
      when p_invoice ? 'due_date_mode' then nullif(trim(p_invoice->>'due_date_mode'), '')
      else due_date_mode
    end,
    due_date = case
      when coalesce(
        nullif(trim(p_invoice->>'due_date_mode'), ''),
        due_date_mode
      ) = 'discussion' then null
      when p_invoice ? 'due_date'
           and nullif(trim(p_invoice->>'due_date'), '') is not null
        then (p_invoice->>'due_date')::date
      else due_date
    end,
    ...
    memo_font_size = coalesce(
      nullif(trim(p_invoice->>'memo_font_size'), ''),
      memo_font_size,
      'normal'
    ),
    document_email = case
...
begin
  ...
  update public.orders set
    ...
    memo_font_size = coalesce(
      nullif(trim(p_order->>'memo_font_size'), ''),
      memo_font_size,
      'normal'
    ),
    document_email = case
...
begin
  ...
  update public.delivery_notes set
    ...
    memo_font_size = coalesce(
      nullif(trim(p_delivery_note->>'memo_font_size'), ''),
      memo_font_size,
      'normal'
    ),
    document_email = case
...
begin
  ...
  update public.receipts set
    ...
    memo_font_size = coalesce(
      nullif(trim(p_receipt->>'memo_font_size'), ''),
      memo_font_size,
      'normal'
    ),
    document_email = case
